...
%>
		...
		<html><head><title>TicTacToe</title></head><body>
		Service is reset!
		<p><form action="play" method="post">
		<input type="submit" name="play" value="Play"></form>
		</body></html>
		<% return; 
	  } %>
      ...
      if (session.getAttribute("new") == null) {
		...
		if (post && move!=null && next==player && winner==0) {
		  ...
		  if (board[i][j]==0) {
			...
			winner = p;
			application.setAttribute("winner", new Integer(p));
			  }
			}
			application.setAttribute("free", new Integer(--free));
		  }
		}
      }
    }
